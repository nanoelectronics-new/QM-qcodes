...
class OPXAnharmonicity(OPX):
    def __init__(self, config: Dict, name: str = "OPXAnharmonicity", host=None, port=None,host_octave=None, port_octave=None, **kwargs):
    ...
    def get_prog(self):
        df = (self.f_stop() - self.f_start()) / (self.n_points()-1)
        ...
        self.parameters['trace_mag_phase'].set_sweep(lo_qubit+self.f_start(),lo_qubit+self.f_stop(),self.n_points())
    ...
    def set_octave(self):
        ...
        self.qm =self.qmm.open_qm(self.config)
        self.qm.octave.set_lo_source('qubit2', OctaveLOSource.Internal)
        self.qm.octave.set_lo_frequency('qubit2', lo_qubit)  # assign the LO inside the octave to element
        self.qm.octave.set_rf_output_gain('qubit2', 0)  # can set gain from -10dB to 20dB
        self.qm.octave.set_rf_output_mode('qubit2', RFOutputMode.trig_normal) # set the behaviour of the RF switch
        self.qm.octave.calibrate_element('qubit2',[(lo_qubit, self.config['elements']['qubit2']['intermediate_frequency'])]) # can provide many pairs of LO & IFs.
        self.qm =self.qmm.open_qm(self.config)
        ...
        self.qm.octave.set_rf_output_gain('qubit', -4)  # can set gain from -10dB to 20dB
        self.qm.octave.set_rf_output_gain('qubit2', -4) # for driving 1-> transition
