...
class OPXMeasPower(OPX):
    ...
    def get_prog(self):
        ...
        self.parameters['trace_mag_phase'].set_sweep(lo_qubit-self.f_start(),lo_qubit-self.f_stop(),self.n_points_f(),
                                                     self.amp_start(),self.amp_stop(),self.n_points_amp(),self.config['waveforms']['readout_wf']['sample'],)
    ...
    def get_res(self):
        if (
            ...
            return {"I": (0,)*n, "Q": (0,)*n, "R": (0,)*n, "Phi": (0,)*n}
             ...
             R = 20*np.log10(np.sqrt(I ** 2 + Q ** 2)/(self.config['waveforms']['readout_wf']['sample']*a[:,np.newaxis]))
