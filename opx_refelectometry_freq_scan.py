...
class OPXSpectrumScan(OPX):
    def __init__(self, config: Dict, name: str = "OPXSpectrumScan", host=None, port=None, **kwargs):
    ...
    def get_prog(self):       
        ...
        with program() as prog:
            ...
            with for_(n, 0, n < n_avg, n + 1):
                with for_(f, self.f_start(), f < self.f_stop()+df/2, f + df):
                    update_frequency("readout", f)
                    measure('readout'*amp(self.amp()), 'resonator', None, demod.full('cos', I, 'out1'), demod.full('sin', Q, 'out1'))
                    save(I, I_st)
                    save(Q, Q_st)
