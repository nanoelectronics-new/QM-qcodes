...
class OPXLineScan(OPX):
        ...
        with program() as prog:
            ...
            with infinite_loop_():
                ...
                with for_(n, 0, n < n_avg, n + 1):
                    measure('cw_reflectometry'*amp(self.amp()), 'readout', None, demod.full('cos', I, 'out1'), demod.full('sin', Q, 'out1'))
                    save(I, I_st)
                    save(Q, Q_st)
    ...
    def get_res(self):
        if (
            ...
            I = self.result_handles.get("I").fetch(self.counter - 1)['value'] / self.readout_pulse_length() * 2**12 * 2
            Q = self.result_handles.get("Q").fetch(self.counter - 1)['value'] / self.readout_pulse_length() * 2**12 * 2
            R = np.sqrt(I ** 2 + Q ** 2)
