...
class OPX(Instrument):
    def __init__(
        ...
        :param cluster_name: Name of the configured cluster.
        ...
        # Open QMM
        if self.qmm is None:
            self.connect_to_qmm(host=host, cluster_name=cluster_name, octave=octave)
        # Set config
        self.set_config(config=config)
        # Open QM
        self.open_qm(close_other_machines)

    def connect_to_qmm(self, host=None, cluster_name=None, octave=None):
        """
        ...
        self.qmm = QuantumMachinesManager(host=host, cluster_name=cluster_name, octave=octave)
        ...
        if self.result_handles is None:
            ...
            for i in range(len(self.results["types"])):
                ...
                if self.results["types"][i] == "IQ":
                    ...
                    #result_handles.get returns np.ndarray with dtype =value
                    out = u.demod2volts(self.result_handles.get(self.results["names"][i]).fetch_all()["value"],
