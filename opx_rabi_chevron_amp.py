...
class IQ2DArray(MultiParameter):
    def __init__(self,
        ...
        self.set_sweep(start_f, stop_f, npts_f, start_amp, stop_amp, npts_amp, 1.0)
#

    def set_sweep(self, start_f: int, stop_f: int, npts_f: int, start_amp: float, stop_amp: float, npts_amp: int, amp: float):
...
class OPXRabiChevron_Amp(OPX):
    def __init__(self, config: Dict, name: str = "OPXRabiChevron_Amp", host=None, port=None,host_octave=None, port_octave=None, **kwargs):
        ...
        self.add_parameter(
            ...
            stop_amp = self.amp_stop(),
            start_amp = self.amp_start(),
            npts_amp = self.n_points_amp(),
    ...
    def get_prog(self):
        ...
        self.parameters['trace_mag_phase'].set_sweep(lo_qubit-self.f_start(),lo_qubit-self.f_stop(),self.n_points_f(),
                                                     self.amp_start(),self.amp_stop(),self.n_points_amp(),self.config['waveforms']['saturation_drive_wf']['sample'],)
        ...
        with program() as prog:
            ...
            with for_(n, 0, n < self.n_avg(), n + 1):
                with for_(a, self.amp_start(), a<self.amp_stop()+self.damp()/2.0, a+self.damp()):
                    with for_(f, self.f_start(), f < self.f_stop()+self.df()/2.0, f + self.df()):                       
                        update_frequency("qubit", f)
                        play("saturation" * amp(a), "qubit", duration=self.duration())
            ...
            with stream_processing():
                I_st.buffer(self.n_points_f()).buffer(self.n_points_amp()).average().save("I")
                Q_st.buffer(self.n_points_f()).buffer(self.n_points_amp()).average().save("Q")
    ...
    def get_res(self):
        if (
            ...
            return {"I": (0,)*n, "Q": (0,)*n, "R": (0,)*n, "Phi": (0,)*n}
             ...
             a= np.arange(self.amp_start(),self.amp_stop()+1e-3,self.damp())
